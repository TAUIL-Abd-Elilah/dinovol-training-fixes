"""Experimental, default-off eager SDPA padding for Dinovol training.

Zero-padding attention heads is established prior art. This helper addresses
the measured 54-wide float16 Dinovol path; it does not establish convergence or
quality equivalence. All native autograd operations remain differentiable.
"""
...
def padded_training_sdpa(query, key, value, attn_mask=None, dropout_p=0.0,
    ...
    """Preserve native SDPA unless opt-in padding enables a fused backend.

    The 54-to-56 zero extension preserves the original attention scale and
    slices only the two added value channels. Gradients use normal pad/slice
    autograd. No backend is forced. Fused kernels can differ numerically.
    ...
    if enabled is True and _eligible(query, key, value, attn_mask, dropout_p, enable_gqa):
        ...
        if not already_supported:
            q, k, v = (F.pad(t, (0, 2)) for t in (query, key, value))
            try:
