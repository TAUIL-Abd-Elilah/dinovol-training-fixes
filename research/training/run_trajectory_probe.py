...
def main():
    ...
    ap.add_argument('--vary-views', action='store_true')
    ...
    if args.audit_every < 1:
        ap.error("audit-every must be positive")
    ...
    try:
        ...
        samples, provenance, fixture_arrays = [], [], []
        ...
        report['audit_protocol'] = {'full_every':args.audit_every,'full_first_and_last':True,
            'intervening':'128 samples per parameter; all full gradients checked finite',
            'audit_helper_sha256':file_sha(trajectory_audit.__file__)}
        report['varying_input_protocol'] = {'enabled':args.vary_views,'rng':'NumPy default_rng(SeedSequence([seed,number]))',
            'origins':'uniform valid integer origins per view and source',
            'masks':'two uniformly selected global rows; 25 percent unique uniform tokens'}
        ...
        if args.vary_views:
            report['input']['construction']['actual_step_views'] = 'Changing origins recorded in each step_input; initial tensor hashes describe only the prototype batch'
            report['protocol']['recipe_deviations'] = report['protocol']['recipe_deviations'].replace('fixed masks documented in input', 'changing deterministic25% masks recorded per step').replace('no stochastic crop augmentation', 'changing deterministic uniform crops; no other augmentation')
        ...
        def varied_batch(number):
            rng = np.random.default_rng(np.random.SeedSequence([args.seed,number]))
            result = dict(original_batch)
            starts = {}
            for key,size,nviews in (('collated_global_crops',args.global_size,2),
                                    ('collated_local_crops',64,n_local_views)):
                views, origins = [], []
                for view in range(nviews):
                    for source_i, array in enumerate(fixture_arrays):
                        origin = rng.integers(0,257-size,size=3).tolist()
                        z,y,x = origin
                        crop = array[z:z+size,y:y+size,x:x+size].astype(np.float32,copy=True)
                        views.append(torch.from_numpy(normalizer.run(crop)).unsqueeze(0).clone())
                        origins.append({'view':view,'source':source_i,'zyx':origin})
                result[key] = torch.stack(views)
                starts[key] = origins
            varying_masks = torch.zeros((4,n_tokens),dtype=torch.bool)
            for row in rng.choice(4,size=2,replace=False):
                varying_masks[int(row),torch.from_numpy(rng.choice(n_tokens,size=n_per_mask,replace=False))] = True
            result['collated_masks'] = varying_masks
            result['mask_indices_list'] = varying_masks.flatten().nonzero().flatten()
            return result, {'origins':starts,'tensor_hashes':{k:hashlib.sha256(v.numpy().tobytes()).hexdigest()
                for k,v in result.items() if isinstance(v,torch.Tensor)}}

        def run_step(number,kind,profile=False):
            nonlocal batch
            step_input = None
            if args.vary_views:
                batch, step_input = varied_batch(number)
            full_audit = number==0 or (number+1)%args.audit_every==0 or number==args.warmups+args.steps-1
            snapshot_fn = snapshot if full_audit else trajectory_audit.snapshot
            before_student = snapshot_fn(trainer.model_module.student)
            before_teacher = snapshot_fn(trainer.model_module.teacher)
            ...
            if full_audit:
                ...
                totals,arrays,layout = trajectory_audit.audit(trainer.model_module.student,
                    trainer.model_module.teacher,before_student,before_teacher,momentum)
            ...
            step_report = {'kind':kind,'number':number,'schedule_step':schedule_step,'elapsed_ms':elapsed_ms if not profile else None,
                ...
                'audit_scope':'full' if full_audit else 'sampled','step_input':step_input}
            ...
            # One shared layout: parameter layout is constant for this trajectory.
            (args.output/'progress.json').write_text(json.dumps({'completed_updates':len(report['steps']), 'latest_checks':checks}),encoding='utf-8')
